...
- **Corpus-agnostic by construction.** The attribute namespace that makes `resolve()`'s grouping fire (so "MRR"/"recurring revenue"/"rev" collide and a contradiction surfaces) has two halves: a universal org/person KERNEL (`reports_to`, `owns`, `role`…), and a DOMAIN vocab that is *induced from the corpus* at ingest and pinned to the store — not hand-authored per company. Point the engine at a new corpus and it derives that corpus's namespace itself; the `new_attribute:<slug>` escape hatch still flags whatever induction misses, so a miss degrades to review, never to a silent synonym. Determinism is preserved by pinning (induce once per corpus change, reuse verbatim after). The name registry is likewise auto-built from the org chart. So "works on a new corpus" is turnkey, not a re-authoring exercise.
...
**Fundamental ones we scoped out (named, not hidden):**
- **The edge is scale-conditional.** The ontology's advantages — correctness when sources contradict, flat per-query cost, recording-time staleness — bite hardest on a corpus large enough that reading it raw fails. At this snapshot's size a strong "read everything" baseline ties us on the visible metrics; our wins live in regimes this corpus doesn't reach. Demonstrating that delta needs a larger, contradiction-dense corpus — out of the time box. The architecture targets that regime; the demo just can't enter it.
- **The edge is self-graded, and routing is tuned to expected questions.** Uplift is measured on our own eval set, and the deterministic hierarchy/contradiction/stance routing is pattern-matched to anticipated phrasings. A blind, paraphrase-heavy held-out set — plus semantic intent detection in place of pattern matching — would prove robustness against unanticipated wording. Not built in this pass.
- *Each is a scope call under a fixed time budget, not a blind spot — and none changes the correctness of what ships, only how far its advantages can be demonstrated here.*
