...
ANTI_ALIASES: list[tuple[str, str, str]] = [
    (
        "Maria Santos (Head of CS, Brasil)",
        "Maria Silva (Head of Sales, Brasil)",
        "org-chart + both interviews open with the interviewer confusing them",
    ),
    (
        "Tan Wei Ming (backend eng, Sao Paulo)",
        "Daniel Tan (VP Engineering)",
        "org-chart + contributors-analysis: 'different people'; relocated SG->SP 2026-02-09",
    ),
    (
        "Pedro Almeida (backend eng, payments)",
        "Sofia Almeida (CRO)",
        "Camila interview: 'mesmo sobrenome so', same surname only, unrelated",
    ),
    (
        "Gabriel Souza (Brasil mobile eng)",
        "Camila Souza (senior backend eng)",
        "contributors-analysis: 'not related to Camila Souza'",
    ),
    (
        "Aaron Wong (Performance Marketing)",
        "Aaron Goh (General Counsel)",
        "two different Aarons; Aaron Wong's core-repo commits are a misattribution",
    ),
    (
        "Aisha Mahmud (FP&A Lead, Finance)",
        "Aisha Yusof (Sales Manager, SEA)",
        "Aisha Mahmud interview meta flags the calendar mix-up explicitly",
    ),
    (
        "Priya Raman (COO)",
        "Priya Devi (HRBP, SEA) / Priya Sharma (Inbound Sales)",
        "three different Priyas; Sarah Ng enumerates them",
    ),
    (
        "HelixPay POS Self-Service (kiosk)",
        "HelixPay POS (card-reader terminal)",
...
NON_PERSON_IDENTITIES: list[tuple[str, str]] = [
    ...
    (
        "Nikita@local",
        "unverified local-machine commit signature, likely an existing eng's 2nd machine",
    ),
    (
        "Aiman Idris (in pos-app commits)",
        "misattribution — Aiman Idris is a CSM, not an engineer; recycled email",
    ),
]
